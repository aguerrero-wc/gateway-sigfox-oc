...
export class ComputedLocation {
  ...
  @ApiPropertyOptional({
    example: 1,
    description: 'Location status (0 = no location)',
  })
  @IsNumber()
  @IsOptional()
  status?: number;
...
export class SigfoxPayloadDto {
  @ApiProperty({
    example: 'service-data-advanced',
    description: 'Type of message from Sigfox',
  })
  @IsString()
  @IsNotEmpty()
  messageType!: string;
  ...
  @ApiPropertyOptional({
    ...
    description:
      'Computed location data (can be empty object with just status)',
  })
  @IsObject()
  @IsOptional()
  @ValidateNested()
